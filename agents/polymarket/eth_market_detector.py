"""
Detector for new ETH updown 15-minute markets.
...
def find_eth_15m_via_clob_api() -> List[Dict]:
    """
    Find ETH 15-minute markets using CLOB API directly.
    ...
    try:
        ...
        # Filter for ETH updown 15-minute markets
        eth_markets = []
        for market in markets:
            # Check various fields for ETH/15m indicators
            question = (market.get("question") or "").lower()
            slug = (market.get("slug") or "").lower()
            description = (market.get("description") or "").lower()
            
            # Look for ETH updown pattern
            is_eth = "ethereum" in question or "eth" in question or "eth" in slug
            is_updown = "up" in question and "down" in question
            is_15m = "15m" in slug or "15m" in question or "15 min" in question
            
            if is_eth and is_updown and is_15m:
                eth_markets.append(market)
                logger.debug(f"Found ETH 15m via CLOB: {market.get('slug', market.get('id'))}")
        
        logger.info(f"Found {len(eth_markets)} ETH 15-minute markets via CLOB API")
        return eth_markets
...
def find_eth_updown_15m_events(limit: int = 100) -> List[Dict]:
    """
    Find ETH updown 15-minute events by searching event slugs.
    Tries multiple approaches since API might filter results.
    
    Returns:
        List of event dicts matching eth-updown-15m pattern
    """
    ...
    if response3.status_code == 200:
        ...
        # Look for ETH markets in question/slug
        for market in markets:
            question = (market.get("question") or "").lower()
            slug = (market.get("slug") or "").lower()
            
            # Check for ETH updown pattern
            if ("ethereum" in question or "eth" in question) and ("up" in question or "down" in question):
                ...
                if "15" in question or "15m" in slug or extract_timestamp_from_slug(slug):
                    ...
                    if event_slug and "eth-updown-15m" in slug:
    ...
    for event in all_events:
        ...
        # Look for eth-updown-15m pattern in slug
        if "eth-updown-15m" in slug:
            eth_events.append(event)
            logger.info(f"Found ETH 15m event: {event.get('slug')}")
        # Also check title/question for ETH updown pattern
        elif ("ethereum" in title or "eth" in title) and ("up" in title or "down" in title):
            # Check if it's 15-minute (look for time ranges or 15m)
            if "15" in title or "15m" in slug or extract_timestamp_from_slug(slug):
                eth_events.append(event)
                logger.info(f"Found ETH 15m event by pattern: {event.get('slug')}")
    
    logger.debug(f"Found {len(eth_events)} ETH 15-minute events out of {len(all_events)} total events")
    return eth_events
...
def get_market_by_event_slug(slug: str) -> Optional[Dict]:
    ...
    Args:
        slug: Event slug (e.g., 'eth-updown-15m-1767571200')
...
def get_latest_eth_15m_market() -> Optional[Dict]:
    """
    Get the most recent ETH 15-minute market.
    ...
    Returns:
        Market dict for the latest ETH updown 15-minute market, or None
    """
    # Approach 1: CLOB API directly (bypasses Gamma API filtering)
    clob_markets = find_eth_15m_via_clob_api()
    if clob_markets:
        ...
        if active_markets:
            ...
            logger.info(f"Found latest ETH 15m market via CLOB API: {latest.get('slug', latest.get('id'))}")
            # Convert to format expected by rest of code
            latest["_event_slug"] = latest.get("slug", f"eth-updown-15m-{latest.get('id')}")
    ...
    events = find_eth_updown_15m_events(limit=200)
    ...
    if events:
        ...
        if markets:
            ...
            logger.info(f"Found latest ETH 15m market via Events API: {latest_event.get('slug')}")
            return market
    
    # Approach 3: Calculate current 15-minute window and construct slug
    # ETH 15-minute markets use timestamps that represent the START of the 15-minute window
    # Example: eth-updown-15m-1767571200 = market for 7:00PM-7:15PM ET (timestamp 1767571200 = 7:00PM start)
    ...
    for window_offset in [0, -1, -2]:
        ...
        slug = f"eth-updown-15m-{test_timestamp}"
...
def get_all_active_eth_15m_markets() -> List[Dict]:
    """
    Get all active ETH 15-minute markets.
    
    Returns:
        List of market dicts for active ETH updown 15-minute markets
    """
    events = find_eth_updown_15m_events(limit=500)
...
def extract_timestamp_from_slug(slug: str) -> Optional[int]:
    """
    Extract timestamp from event slug like 'eth-updown-15m-1767571200'.
    
    Returns:
        Timestamp as int, or None if not found
    """
    # Pattern: eth-updown-15m-{timestamp}
    match = re.search(r'eth-updown-15m-(\d+)', slug)
    if match:
