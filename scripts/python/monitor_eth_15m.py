"""
Automatically monitor new ETH updown 15-minute markets.
Detects new markets by checking event slugs and starts monitoring them.

Usage:
    python scripts/python/monitor_eth_15m.py
"""
import asyncio
import logging
import sys
...
from agents.polymarket.eth_market_detector import (
    get_latest_eth_15m_market,
    get_all_active_eth_15m_markets,
...
class ETH15mMonitor:
    """Monitor for new ETH 15-minute markets."""
    ...
    async def _check_for_new_markets(self):
        """Check for new ETH 15-minute markets."""
        logger.info("Checking for new ETH updown 15-minute markets...")
        
        # Try to get latest market (uses multiple detection approaches)
        from agents.polymarket.eth_market_detector import get_latest_eth_15m_market
        latest_market = get_latest_eth_15m_market()
        
        markets = []
        if latest_market:
            markets = [latest_market]
            logger.info(f"Found latest ETH 15-minute market via detection")
        else:
            # Fallback: try getting all active markets
            markets = get_all_active_eth_15m_markets()
            logger.info(f"Found {len(markets)} active ETH 15-minute markets via search")
        ...
        for market in markets:
            ...
            logger.info(f"Found new ETH 15-minute market: {market.get('question', event_slug)[:60]}...")
    ...
    async def _start_monitoring(self, token_ids: list, market_info: dict):
        ...
        # Fallback to polling (or use if no wallet key)
        from agents.polymarket.orderbook_poller import OrderbookPoller
    ...
    async def run(self):
        ...
        self.running = True
        logger.info(f"Starting ETH 15-minute market monitor (check interval: {self.check_interval}s)")
